...
function Layout({ logOut, profile, user }) {
  ...
  const saveNote = async (note, index) => {
    ...
    let url =`https://6ad6jjoqaym6xhmmfuwk7yn65i0rtgvv.lambda-url.ca-central-1.on.aws/save?email=${profile.email}&id=${note.id}`
    fetch(url,
      {
        mode:   'no-cors',
        method: "POST",
  ...
  useEffect(() => {
    const getNoteEffect = async () => {
      if (profile.email) {
        let url =`https://jt6zm2ojujrmfzopqoh3zomszu0eayua.lambda-url.ca-central-1.on.aws/get?email=${profile.email}`
  ...
  const deleteNote = async (id, index) => {
    ...
    const rev = await fetch(
      `https://4275zhydohuwf2fgl2vldyahei0htxsn.lambda-url.ca-central-1.on.aws/`,
      {
        mode:   'no-cors',
        method: "POST",
        headers: {
